...
import com.us.clique.databinding.ActivityYourExperienceDetailsBinding;
...
public class YourExperienceDetailsActivity extends AppCompatActivity implements View.OnClickListener, EventsAroundYouDetailsInterface {
    BottomNavigationViewModel bottomNavigationViewModel;
    @Inject
    BottomNavigationFactory bottomNavigationFactory;
    ActivityYourExperienceDetailsBinding binding;
    UserSessionManager session;
    ProgressDialog loading;
    ProgressDialog loading1;
    ProgressDialog loading2;
    ProgressDialog loading3;
    String access_token = " ",experinceEventsId,userId,eventUserId;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = DataBindingUtil.setContentView(YourExperienceDetailsActivity.this,
                R.layout.activity_your_experience_details);
        bottomNavigationViewModel = new ViewModelProvider(this, bottomNavigationFactory).get(BottomNavigationViewModel.class);
        binding.setYoureExperinceDetails(bottomNavigationViewModel);
...
//        }
    ...
    private void setData(){
        for (int i=0;i<experinceList.size();i++){
            String  members,timeDate,NameAndAge;
            eventUserId = experinceList.get(i).getEventUserId();
           // requestUserId = experinceList.get(i).;
            NameAndAge = experinceList.get(i).getUserName()+","+experinceList.get(i).getAge();
            members = experinceList.get(i).getMinimumMember()+","+experinceList.get(i).getMaximumMember();
            timeDate = experinceList.get(i).getEventTime()+","+experinceList.get(i).getEventDateFormat();
            ...
            binding.tvUserName.setText(NameAndAge);
...
//                    RequestRecycler();
            ...
            case R.id.btn_requestToJoin:
                ...
                binding.btnRequestToJoin.setText("Pending");
                break;
            case R.id.btn_messages:
                callRequestToMessageApi();
                binding.btnMessages.setText("Pending");
                break;
        }
    }
